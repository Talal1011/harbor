...
test("external backfills cannot manufacture fresh activity for dismissed items", () => {
  for (const provider of ["trakt", "simkl"]) {
    const source = read(`src/lib/${provider}/playback.ts`);
    assert.match(source, /existing <= 0 && !isCwDismissed\(item\)/);
    assert.match(source, /pct < 1 \|\| pct > 98/);
  }
});
